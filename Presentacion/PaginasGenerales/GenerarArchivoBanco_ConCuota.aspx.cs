...
public partial class GenerarArchivoBanco_ConCuota : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            if (!Page.IsPostBack)
            {
                ...
                if (cntId.SelectedIndex != 0)
                {
                    int TipoConc = Convert.ToInt32(cntId.SelectedValue);
                    DataTable dt1 = new DataTable();
                    dt1 = ocnConceptos.ObtenerListaPorUnTipoConcepto("[Seleccionar...]", insId, Convert.ToInt32(cntId.SelectedValue), Convert.ToInt32(conAnioLectivo.Text));
                    if (dt1.Rows.Count > 1)
                    {
                        ConceptoId.DataValueField = "Valor";
                        ConceptoId.DataTextField = "Texto";
                        ConceptoId.DataSource = (new GESTIONESCOLAR.Negocio.Conceptos()).ObtenerListaPorUnTipoConcepto("[Seleccionar...]", insId, Convert.ToInt32(cntId.SelectedValue), Convert.ToInt32(conAnioLectivo.Text));
                        ConceptoId.DataBind();
                    }
                }
                ...
                if (BcoAdhId.SelectedValue == "2")
                {
                    if (mesHoy == "2")
                    {
                        if ((Convert.ToInt32(diaHoy) >= 23) & (Convert.ToInt32(diaHoy) <= 26)) // SOLO POR EL 2024
                        {
                    ...
                    {
                        if ((Convert.ToInt32(diaHoy) >= 16) & (Convert.ToInt32(diaHoy) <= 23))
                        {
                        ...
                        else
                        {
                            if ((Convert.ToInt32(diaHoy) >= 1) & (Convert.ToInt32(diaHoy) <= 13))
                            {
                                ...
                                FchImputar.Text = "15/" + Convert.ToString(Convert.ToInt32(mesHoy)) + "/" + DateTime.Now.ToString("yyyy");
...
"</div>";
    ...
    protected void cntId_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            if (cntId.SelectedIndex != 0)
            {
                int TipoConc = Convert.ToInt32(cntId.SelectedValue);
                DataTable dt = new DataTable();
                dt = ocnConceptos.ObtenerListaPorUnTipoConcepto("[Seleccionar...]", insId, Convert.ToInt32(cntId.SelectedValue), Convert.ToInt32(conAnioLectivo.Text));
                if (dt.Rows.Count > 1)
                {
                    ConceptoId.DataValueField = "Valor";
                    ConceptoId.DataTextField = "Texto";
    ...
    protected void btnAceptar_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            else
            {
                ...
                if (mesHoy == "2")
                {
                    if ((Convert.ToInt32(diaHoy) >= 23) & (Convert.ToInt32(diaHoy) <= 26))
                    {
                        ddlDesde.SelectedValue = "1";
                        ddlDisparo.SelectedValue = "1";
                        //FchImputar.Text = "01/03/" + DateTime.Now.ToString("yyyy");
...
MESSAGE:<br>" + oError.Message + "<br><br>EXCEPTION:<br>" + oError.InnerException + "<br><br>TRACE:<br>" + oError.StackTrace + "<br><br>TARGET:<br>" + oError.TargetSite +
    ...
    protected void BcoAdhId_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (BcoAdhId.SelectedValue == "2")
        {
            ...
            if (mesHoy == "2")
            {
                ...
                {
                    alerError.Visible = true;
                    lblError.Text = "En febrero solo se puede enviar el archivo hasta la fecha indicada por autoridades..";
                }
            }

            else
            {
                if ((Convert.ToInt32(diaHoy) >= 15) & (Convert.ToInt32(diaHoy) <= 23))
                {
                    ddlDesde.SelectedValue = Convert.ToString(Convert.ToInt32(mesHoy) - 3 + 2); // 3 es mes de Inicio
                    FchImputar.Text = "01/" + Convert.ToString(Convert.ToInt32(mesHoy) + 1) + "/" + DateTime.Now.ToString("yyyy");
    ...
    protected void txtDesde_TextChanged(object sender, EventArgs e)
    {
        ...
        if (mesHoy == "2")
        {
            if ((Convert.ToInt32(diaHoy) >= 23) & (Convert.ToInt32(diaHoy) <= 26)) // SOLO PARA 2024
            {
                ddlDesde.SelectedValue = "1";
                FchImputar.Text = "01/03/" + DateTime.Now.ToString("yyyy");
