...
public partial class MovimientoCambioCursoTerciario : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!Page.IsPostBack)
            {
                ...
                NivelID2.SelectedValue = "4";
                NivelID.Enabled = false;
                NivelID2.Enabled = false;
                ...
                else// es primario inicial o secundario
                {
                    ...
                    dtCarrera = ocnCarrera.ObtenerUnoxNivel(Convert.ToInt32(NivelID.SelectedValue));
                    if (dtCarrera.Rows.Count > 0)
                    {
                        car = Convert.ToInt32(dtCarrera.Rows[0]["Id"].ToString());
                ...
                #region PageIndex
                int PageIndex = 0;
                if (this.Session["MovimientoCambioCursoTerciario.PageIndex"] == null)
                {
                    Session.Add("MovimientoCambioCursoTerciario.PageIndex", 0);
                }
                else
                {
                    PageIndex = Convert.ToInt32(Session["MovimientoCambioCursoTerciario.PageIndex"]);
                }
                #endregion
...
"</div>";
    ...
    private void GrillaCargar(int PageIndex)
    {
        try
        {
            ...
            Session["MovimientoCambioCursoTerciario.PageIndex"] = PageIndex;
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                if (Convert.ToInt32(dt.Rows[0]["SinPC"].ToString()) == 0)//TIENE CARRERA Y PLAN? 0=SUPERIOR
                {
                    ...
                    carId2.DataValueField = "Valor";
                    carId2.DataTextField = "Texto";
                    carId2.DataSource = (new GESTIONESCOLAR.Negocio.Carrera()).ObtenerListaxtcaId("[Seleccionar...]", Convert.ToInt32(NivelID.SelectedValue));
                    carId2.DataBind();
                    carId2.Enabled = true;
            ...
            if (lblCantidadRegistros2.Text != "")
            {
                ...
                if (dt4.Rows.Count > 0)
                {
                    ...
                    foreach (GridViewRow row in GrillaAlumnos2.Rows)
                    {
                        ...
                        int curIdDestino = Convert.ToInt32((curId2.SelectedValue.Trim() == "" ? "0" : curId2.SelectedValue));
                        ...
                        if (AnioCursado2.Text.Trim() == AnioCursado.Text.Trim())//Cambio de Curso
                        {
                            ...
                            DataTable dtInsCurTerTraer = new DataTable();
                            dtInsCurTerTraer = ocnInscripcionCursadoTerciario.ObtenerxicuId(icuId);

                            if (dtInsCurTerTraer.Rows.Count == 0)
                            {
                                ...
                                ocnInscripcionCursado.curId = curIdDestino;
                            ...
                            {
                                alerError.Visible = true;
                                lblError.Text = "Tiene Materias asignadas.. No puede realizar cabio de curso. (Opción: eliminar materias.)";
                                return;
                            }
                        }
                        else
