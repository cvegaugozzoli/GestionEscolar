            ...
            using (StreamReader ReaderObject = new StreamReader(FileToRead))
            {
                ...
                while ((Line = ReaderObject.ReadLine()) != null)
                {
                    ...
                    if (PL == 'D')
                    {
                        ...
                        {
                            dt6 = ocnInscripcionConcepto.ObtenerUnoxaluxcuotaximporteDetallePat(Convert.ToInt32(aluIdNum), numCuotaNum, ImporteRealNum, AnioCursado);// obtengo inscripcion Cocepto que  exista en comprobante detalle
                            dt3 = ocnAlumno.ObtenerUno(Convert.ToInt32(aluIdNum));
                            if (dt6.Rows.Count > 0)
                            {
                                lblColegioId.Text = Convert.ToString(dt6.Rows[0]["insId"].ToString());
                                if (dt3.Rows.Count > 0)
                                {
                                    int icoId = Convert.ToInt32(dt6.Rows[0]["Id"].ToString());
