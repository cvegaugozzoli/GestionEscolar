...
"</div>";
    ...
    private void GrillaCargar(int PageIndex)
    {
        try
        {
            ...
            dt = new DataTable();
            int perIdDocente = 23; // profesor terciario
            int insIdTraer = Convert.ToInt32(Session["_Institucion"] );

            dt = ocnUsuario.ObtenerTodoBuscarxNombrexPerTerc(Nombre.Text.Trim(), insIdTraer);
...
"</div>";
    ...
    protected void btnEliminarAceptar_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            int perId = Convert.ToInt32(((HyperLink)((GridViewRow)((Button)sender).Parent.Parent).Cells[7].Controls[1]).Text);
            int usuId = Convert.ToInt32(((HyperLink)((GridViewRow)((Button)sender).Parent.Parent).Cells[8].Controls[1]).Text);                

            DataTable dt = ocnUsuarioPerfil.ObtenerTodoxusuId(usuId);
            if (dt.Rows.Count == 1) // si hay un solo registro elimino todo: Docente, Usuario, Usuario Perfil
            {
                //ocnDocentes.Eliminar(Id);
                //ocnUsuario.Eliminar(usuId);
                //int upeId = Convert.ToInt32(dt.Rows[0]["Id"]);
                //ocnUsuarioPerfil.Eliminar(upeId);
            }
            else
            {
                if (dt.Rows.Count > 1) // si hay mas de un registro elimino solo:  Usuario Perfil
                {
                    //DataTable dt2 = ocnUsuarioPerfil.ObtenerTodoxusuIdxPerId(usuId,perId);
                    //int upeId = Convert.ToInt32(dt2.Rows[0]["Id"]);
                    //ocnUsuarioPerfil.Eliminar(upeId);
                }
            }
