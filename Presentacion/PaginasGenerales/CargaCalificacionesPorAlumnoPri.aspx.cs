    ...
    protected void OnRowUpdating(object sender, GridViewUpdateEventArgs e)
    {
        try
        {
            ...
            if (!Session["_perId"].ToString().Equals("10") && !Session["_perId"].ToString().Equals("9"))
            {
                ...
                string PTrim2 = ProcesarNota((TextBox)row.FindControl("txtPTrim"), "Primer Trimestre", ref errores, esInasistencia);
                string STrim2 = ProcesarNota((TextBox)row.FindControl("txtSTrim"), "Segundo Trimestre", ref errores, esInasistencia);
                string TTrim2 = ProcesarNota((TextBox)row.FindControl("txtTTrim"), "Tercer Trimestre", ref errores, esInasistencia);
                ...
                ocnRegistracionNota.ActualizarPrimaria(Id, PTrim2, STrim2, TTrim2, PAnual2, NotaDic2, NotaMar2, renCalfDef2, ahora, ahora, usuId, usuId);

                GrillaNota.EditIndex = -1;
                int PageIndex2 = Convert.ToInt32(Session["CargaCalificacionesPorAlumnoPri.PageIndex"]);
                GrillaNota.DataSource = dt5;
                GrillaNota.PageIndex = PageIndex2;
                GrillaNota.DataBind();
            }

            int PageIndex = Convert.ToInt32(Session["CargaCalificacionesPorAlumnoPri.PageIndex"]);
            GrillaCargar(PageIndex);
        }
        catch (Exception oError)
        {
...
"</div>";
    ...
    protected void ButtonAsignar_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                int anioTemporal;
                if (!int.TryParse(Request.QueryString["Anio"], out anioTemporal))
                {
                    LblMensajeErrorGrilla.Text = "El año proporcionado no es válido.";
                    return;
                }
                anioCur = anioTemporal;
            }
...
MESSAGE:<br>" + oError.Message + "<br><br>EXCEPTION:<br>" + oError.InnerException + "<br><br>TRACE:<br>" + oError.StackTrace + "<br><br>TARGET:<br>" + oError.TargetSite +
    ...
    protected void BtnMostrar_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                this.GrillaPrevia.DataSource = dt4;
                Convert.ToInt32(Session["CargaCalificacionesPorAlumnoPri.PageIndex"]);
                this.GrillaPrevia.PageIndex = PageIndex;
                this.GrillaPrevia.DataBind();
...
MESSAGE:<br>" + oError.Message + "<br><br>EXCEPTION:<br>" + oError.InnerException + "<br><br>TRACE:<br>" + oError.StackTrace + "<br><br>TARGET:<br>" + oError.TargetSite +
    ...
    protected void GrillaBuscar_PageIndexChanging(object sender, GridViewPageEventArgs e)
    {
        try
        {
            if (Session["CargaCalificacionesPorAlumnoPri.PageIndex"] != null)
            {
                Session["CargaCalificacionesPorAlumnoPri.PageIndex"] = e.NewPageIndex;
            }
            else
            {
                Session.Add("CargaCalificacionesPorAlumnoPri.PageIndex", e.NewPageIndex);
            }
        ...
        try
        {
            if (e.CommandName != "Sort" && e.CommandName != "Page" && e.CommandName != "")
            {
                ...
                if (e.CommandName == "Select")
                {
                    ...
                    {
                        alerError.Visible = true;
                        lblError.Text = "Ud. solo puede acceder a la información del Nivel Primario";
                        this.GrillaBuscar.DataSource = null;
                        this.GrillaBuscar.PageIndex = 0;
                        this.GrillaBuscar.DataBind();
                        panelCalif.Visible = false;
    ...
    private void GrillaBuscarCargar(int PageIndex)
    {
        try
        {
            ...
            if (rbNombre.Checked==true)
            {
                ...
                // Filtro: carId = 2 y excluir curId en (89, 88, 90, 91, 92, 132)
                if (dt != null && dt.Columns.Contains("carId") && dt.Columns.Contains("curId"))
                {
                    string filtro = "carId = 2 AND curId NOT IN (89, 88, 90, 91, 92, 132)";
                    ...
                    {
                        dt.Rows.Clear(); // Si no hay coincidencias, dejar vacío
                    }
                }
                ...
                {
                    alerError.Visible = true;
                    lblError.Text = "No se encuentra Alumno con esa descripción..";
                    pnelAlumnoSeleccionado.Visible = false;
                    return;
                }
            }
            else
            {
                ...
                {
                    alerError.Visible = true;
                    lblError.Text = "No se encuentra Alumno con ese DNI..";
                    pnelAlumnoSeleccionado.Visible = false;
                }
            }
        }
    ...
    protected void btnGuardar_Click(object sender, EventArgs e)
    {
        ...
        if (!Session["_perId"].ToString().Equals("10") && !Session["_perId"].ToString().Equals("9"))
        {
            ...
            foreach (GridViewRow row in GrillaNota.Rows)
            {
                ...
                //if (txtPTrim != null && txtPTrim.Enabled && !string.IsNullOrWhiteSpace(txtPTrim.Text))
                {
