...
public partial class LlamadosExamenConsulta : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!Page.IsPostBack)
            {
                this.Master.TituloDelFormulario = " LlamadosExamen - Consulta";

                //if (this.Session["_Autenticado"] == null) Response.Redirect("Login.aspx", true);

                #region PageIndex
                int PageIndex = 0;
                if (this.Session["LlamadosExamenConsulta.PageIndex"] == null)
                {
                    Session.Add("LlamadosExamenConsulta.PageIndex", 0);
                }
                else
                {
                    PageIndex = Convert.ToInt32(Session["LlamadosExamenConsulta.PageIndex"]);
                }
                #endregion
...
MESSAGE:<br>" + oError.Message + "<br><br>EXCEPTION:<br>" + oError.InnerException + "<br><br>TRACE:<br>" + oError.StackTrace + "<br><br>TARGET:<br>" + oError.TargetSite +
    ...
    protected void btnNuevo_Click(object sender, EventArgs e)
    {
	    try
	    {
            ...
            Response.Redirect("LlamadosExamenRegistracion.aspx?Id=0", false);
	    }
	    catch (Exception oError)
	    {
            lblMensajeError.Text = @"<div class=""alert alert-danger alert-dismissable"">
...
"</div>";
    ...
    private void GrillaCargar(int PageIndex)
    {
        try
        {
            Session["LlamadosExamenConsulta.PageIndex"] = PageIndex;
...
"</div>";
    ...
    protected void btnEliminarAceptar_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            dtTurno = ocnTurnoExamen.ObtenerTodoxLlamado(Id);

            if (dtTurno.Rows.Count == 0)
            {
                ocnLlamadosExamen.EliminarxActivo(Id, Master.usuId, DateTime.Now);
            }
            else
            {
                alerError.Visible = true;
                lblError.Text = "No puedes eliminar un Llamados que ya estan asociados a Turnos..";
            }

            //ocnLlamadosExamen.EliminarxActivo(Id, this.Master.usuId, DateTime.Now);
