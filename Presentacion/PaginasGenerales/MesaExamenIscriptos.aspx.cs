...
public partial class MesaExamenIscriptos : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!Page.IsPostBack)
            {
                this.Master.TituloDelFormulario = "Inscriptos en Mesa de Examen";
                if ((Session["_perId"].ToString() == "1") || (Session["_perId"].ToString() == "6") || (Session["_perId"].ToString() == "9") || (Session["_perId"].ToString() == "18") || (Session["_perId"].ToString() == "22") || Session["_perId"].ToString() == "21" || Session["_perId"].ToString() == "24")  // Si es administrador o Director veo todas las carreras
                {
                    ...
                    if (dt2.Rows.Count > 0)
                    {
                        carIdDD.DataValueField = "Valor";
                        carIdDD.DataTextField = "Texto";
                        carIdDD.DataSource = (new GESTIONESCOLAR.Negocio.Carrera()).ObtenerListaxtcaId("[Seleccionar...]", niv);
                        carIdDD.DataBind();
                ...
                txtAnio.Text = Convert.ToString(DateTime.Now.Year);
                extIdDD.DataValueField = "Valor"; extIdDD.DataTextField = "Texto"; extIdDD.DataSource = (new GESTIONESCOLAR.Negocio.InscripcionExamenTipo()).ObtenerLista("[Seleccionar...]"); extIdDD.DataBind();
...
"</div>";
    ...
    {
        if (carIdDD.SelectedIndex != 0)
        {

            //ClubB.Negocio.Evento ocnEvento = new ClubB.Negocio.Evento();
            DataTable dt = new DataTable();
            dt = ocnPlanEstudio.ObtenerListaPorUnaCarrera("[Seleccionar...]", Convert.ToInt32(carIdDD.SelectedValue));
            if (dt.Rows.Count > 0)
            {
                plaIdDD.DataValueField = "Valor";
                plaIdDD.DataTextField = "Texto";
                plaIdDD.DataSource = (new GESTIONESCOLAR.Negocio.PlanEstudio()).ObtenerListaPorUnaCarrera("[Seleccionar...]", Convert.ToInt32(carIdDD.SelectedValue));
                plaIdDD.DataBind();
            }
        }
...
MESSAGE:<br>" + oError.Message + "<br><br>EXCEPTION:<br>" + oError.InnerException + "<br><br>TRACE:<br>" + oError.StackTrace + "<br><br>TARGET:<br>" + oError.TargetSite +
    ...
    {
        if (plaIdDD.SelectedIndex > 0)
        {

            //ClubB.Negocio.Evento ocnEvento = new ClubB.Negocio.Evento();
            DataTable dt = new DataTable();
            dt = ocnCurso.ObtenerListaPorUnPlanEstudio("[Seleccionar...]", Convert.ToInt32(plaIdDD.SelectedValue));
            if (dt.Rows.Count > 0)
            {
                curIdDD.DataValueField = "Valor";
                curIdDD.DataTextField = "Texto";
                curIdDD.DataSource = (new GESTIONESCOLAR.Negocio.Curso()).ObtenerListaPorUnPlanEstudio("[Seleccionar...]", Convert.ToInt32(plaIdDD.SelectedValue));
                curIdDD.DataBind();
    ...
    private void curIdCargar()
    {
        if (curIdDD.SelectedIndex > 0)
        {
            ...
            {
                escId.DataValueField = "Valor"; escId.DataTextField = "Texto"; escId.DataSource = (new GESTIONESCOLAR.Negocio.EspacioCurricular()).ObtenerListaPorUnCurso2("[Seleccionar...]", Convert.ToInt32(curIdDD.SelectedValue), insId); escId.DataBind();
            }
            else
            {
                ...
                {
                    escId.DataValueField = "Valor"; escId.DataTextField = "Texto"; escId.DataSource = (new GESTIONESCOLAR.Negocio.UsuarioEspacioCurricular()).ObtenerListaxupeIdyCur("[Seleccionar...]", upeId, Convert.ToInt32(curIdDD.SelectedValue)); escId.DataBind();
                }
            }
        }
    }
    ...
    private void GrillaCargar(int PageIndex)
    {
        try
        {
            ...
            Int32 car = 0;
            if (carIdDD.SelectedValue.ToString() != "" & carIdDD.SelectedValue.ToString() != "0")
            {
                car = Convert.ToInt32(carIdDD.SelectedValue.ToString());
            }
            Int32 pla = 0;
            if (plaIdDD.SelectedValue.ToString() != "" & plaIdDD.SelectedValue.ToString() != "0")
            {
                pla = Convert.ToInt32(plaIdDD.SelectedValue.ToString());
            }
            Int32 cur = 0;
            if (curIdDD.SelectedValue.ToString() != "" & curIdDD.SelectedValue.ToString() != "0")
            {
                cur = Convert.ToInt32(curIdDD.SelectedValue.ToString());
            }
            Int32 escId2 = 0;
            ...
            if (aplicafiltrofechaCH.Checked)
            {
            ...
            int itpId2 = Convert.ToInt32(extIdDD.SelectedValue);
            dt = new DataTable();
            dt = ocnInscripcionExamen.ObtenerxConsultaME(Convert.ToInt32(TurnoId.SelectedValue), Convert.ToInt32(carIdDD.SelectedValue), pla,
...
MESSAGE:<br>" + oError.Message + "<br><br>EXCEPTION:<br>" + oError.InnerException + "<br><br>TRACE:<br>" + oError.StackTrace + "<br><br>TARGET:<br>" + oError.TargetSite +
    ...
    protected void btnEliminarAceptar_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            ocnInscripcionExamen.EliminarActivo(Id);
...
MESSAGE:<br>" + oError.Message + "<br><br>EXCEPTION:<br>" + oError.InnerException + "<br><br>TRACE:<br>" + oError.StackTrace + "<br><br>TARGET:<br>" + oError.TargetSite +
        ...
        if (dt.Rows.Count > 0)
        {
            if (Convert.ToInt32(dt.Rows[0]["SinPC"].ToString()) == 0)//TIENE CARRERA Y PLAN? 0=SUPERIOR
            {
                ...
                if (dt2.Rows.Count > 0)
                {
                    carIdDD.DataValueField = "Valor";
                    carIdDD.DataTextField = "Texto";
                    carIdDD.DataSource = (new GESTIONESCOLAR.Negocio.Carrera()).ObtenerLista("[Seleccionar...]");
                    carIdDD.DataBind();
            ...
            else// es primario inicial o secundario
            {
                ...
                curIdDD.DataValueField = "Valor";
                curIdDD.DataTextField = "Texto";
                curIdDD.DataSource = (new GESTIONESCOLAR.Negocio.Curso()).ObtenerListaPorUnPlanEstudio("[Seleccionar...]", plaIdO);
                curIdDD.DataBind();
                carIdDD.Enabled = false;
                plaIdDD.Enabled = false;
                carIdDD.SelectedValue = "0";
                plaIdDD.SelectedValue = "0";
            }
        }
        else
        {
    ...
    protected void BtnImprimir_Click(object sender, EventArgs e)
    {
        ...
        if (TurnoId.SelectedValue.ToString() != "" & TurnoId.SelectedValue.ToString() != "0")
        {
            turId = Convert.ToInt32(TurnoId.SelectedValue.ToString());
        }     
      
        Int32 curId = 0;
        if (curIdDD.SelectedValue.ToString() != "" & curIdDD.SelectedValue.ToString() != "0")
        {
            curId = Convert.ToInt32(curIdDD.SelectedValue.ToString());
        }
        Int32 espaciocurricular = 0;
        if (escId.SelectedValue.ToString() != "" & escId.SelectedValue.ToString() != "0")
        {
            espaciocurricular = Convert.ToInt32(escId.SelectedValue.ToString());
        }
        Int32 aplicafiltrofecha = 0;
        if (aplicafiltrofechaCH.Checked)
        {
            aplicafiltrofecha = 1;
        }
        ...
        {
            txtAnio.Text = Convert.ToString(DateTime.Now.Year);
            aniocursado = Convert.ToInt32(txtAnio.Text);
        }
        else
        {
            aniocursado = Convert.ToInt32(txtAnio.Text);
        ...
        DateTime fechaDesde = Convert.ToDateTime(ixaFechaExamenDesde.Text.ToString("dd-MM-yyyy"));
        DateTime fechaHasta = Convert.ToDateTime(ixaFechaExamenHasta.Text.ToString("dd-MM-yyyy"));
           

        int itpId = Convert.ToInt32(extIdDD.SelectedValue);

        String NomRep = "MesaExamenInscriptos.rpt";

        FuncionesUtiles.AbreVentana("Reporte.aspx?turId="+turId+"&curId="+curId+"&aniocursado="+aniocursado+
            "&espaciocurricular="+espaciocurricular+"&fechaDesde="+fechaDesde+"&fechaHasta="+
            fechaHasta+"&aplicafiltrofecha="+aplicafiltrofecha+"&itpId="+itpId+"&NomRep="+NomRep);
